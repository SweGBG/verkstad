      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: "1000px", margin: "0 auto", padding: "120px 24px 80px" }}>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "16px" }}>
            ...
            <span style={{ color: "rgba(220,50,30,0.9)", fontSize: "11px", fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase" }}>NordDäck — 45 min</span>
          </div>
          <h1 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "clamp(56px, 8vw, 96px)", fontWeight: 900, color: "#fff", textTransform: "uppercase", lineHeight: 0.95, marginBottom: "24px" }}>
            Olje<span style={{ color: "rgba(220,50,30,0.9)" }}>byte</span>
          </h1>
          <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "17px", lineHeight: 1.8, maxWidth: "600px", marginBottom: "48px" }}>
            Regelbundet oljebyte är det bästa du kan göra för din motor. Vi använder rätt olja för just din bil och kontrollerar filter, nivåer och läckage — allt på 45 minuter.
          </p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} style={{ marginBottom: "60px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "16px" }}>
            {[
              { typ: "Mineralol ja", spec: "Äldre bensinmotorer", pris: "795 kr" },
              { typ: "Halvsyntetisk", spec: "Diesel & bensin", pris: "995 kr" },
              { typ: "Helsyntetisk", spec: "Moderna motorer", pris: "1 295 kr" },
            ].map((p, i) => (
              <motion.div key={p.typ} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + i * 0.1 }}
                style={{ background: i === 2 ? "rgba(220,50,30,0.06)" : "rgba(255,255,255,0.02)", border: `1px solid ${i === 2 ? "rgba(220,50,30,0.3)" : "rgba(255,255,255,0.06)"}`, borderRadius: "12px", padding: "24px", position: "relative", overflow: "hidden" }}
              >
                {i === 2 && <div style={{ position: "absolute", top: "12px", right: "12px", background: "rgba(220,50,30,0.9)", color: "#fff", fontSize: "9px", fontWeight: 700, letterSpacing: "2px", padding: "3px 8px", borderRadius: "4px", textTransform: "uppercase" }}>Populär</div>}
          ...
          <p style={{ color: "rgba(255,255,255,0.2)", fontSize: "12px", marginTop: "12px" }}>* Olja & filter ingår i priset.</p>
        </motion.div>
